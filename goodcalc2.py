...
def gcf(a,b):
    alist = factors(a).split()
    blist = factors(b).split()
    large = 1
    for i in alist:
        if i in blist and int(i) > large:
            large = int(i)
    return large
...
def main(TI, ID, answer, j):  #The Input, Initial Data
    ...
    try:
        if origin == 'choose':
            ...
            ANSWER = ' '.join(
                [str(main('/choose ' + str(thein[1]) + ' ' + str(i), ID, answer)) for i in range(thein[1] + 1)])
        elif origin == 'perfect':
